 ...
 * </P>
 */
public class PApplet extends Applet implements PConstants, Runnable,
  ...
  /**
   * Modifier flags for the shortcut key used to trigger menus. (Cmd on Mac OS
   * X, Ctrl on Linux and Windows)
   */
  static public final int MENU_SHORTCUT = Toolkit.getDefaultToolkit()
   ...
   * Running full screen across multiple displays isn't particularly supported,
   * and requires more monkeying with the values. This probably can't/won't be
   * fixed until/unless I get a dual head system.
   * <P>
   * Note that this won't update if you change the resolution of your screen
   * once the the applet is running.
   * <p>
   * This variable is not static, because future releases need to be better at
   ...
   * platforms, Mac OS X 10.3 can do any height but requires at least 128 pixels
   * width. Windows XP has another set of limitations. And for all I know, Linux
   * probably lets you make windows with negative sizes.
   */
   ...
   * <P>
   * When used with OpenGL or Java2D, this value will be null until loadPixels()
   * has been called.
   */
  public int pixels[];
   ...
   * mousePressed() and friends, it's updated each time an event comes through.
   * Be sure to use only one or the other type of means for tracking pmouseX and
   * pmouseY within your sketch, otherwise you're gonna run into trouble.
   */
  public int pmouseX, pmouseY;

  /**
   * previous mouseX/Y for the draw loop, separated out because this is separate
   * from the pmouseX/Y when inside the mouse event handlers.
   */
  protected int dmouseX, dmouseY;

  /**
   * pmouseX/Y for the event handlers (mousePressed(), mouseDragged() etc) these
   * are different because mouse events are queued to the end of draw, so the
   * previous position has to be updated on each event, as opposed to the
   ...
   * (integrated) over several frames. As such, this value won't be valid until
   * after 5-10 frames.
   */
   ...
   * allocated.
   * <p/>
   * This also attempts to call PApplet.stop(), in case there was an inadvertent
   * override of the stop() function by a user.
   * <p/>
   * destroy() supposedly gets called as the applet viewer is shutting down the
   * applet. stop() is called first, and then destroy() to really get rid of
   * things. no guarantees on when they're run (on browser quit, or when moving
   ...
   * 
   * The size() method should *only* be called from inside the setup() or draw()
   * methods, so that it is properly run on the main animation thread. To change
   * the size of a PApplet externally, use setSize(), which will update the
   * component size, and queue a resize of the renderer as well.
   */
   ...
   * best to use <A HREF="http://dev.processing.org/reference/everything/javadoc/processing/core/PGraphics3D.html"
   ...
   * HREF="http://dev.processing.org/reference/core/javadoc/processing/core/PImage.html#save(java.lang.String)"
   * >PImage.save()</A>.
   * </UL>
   */
  ...
  /**
   * Mouse has been pressed, and should be considered "down" until
   * mouseReleased() is called. If you must, use int button =
   * mouseEvent.getButton(); to figure out which button was clicked. It will be
   * one of: MouseEvent.BUTTON1, MouseEvent.BUTTON2, MouseEvent.BUTTON3 Note,
   * however, that this is completely inconsistent across platforms.
   */
  public void mousePressed() {
  }
  ...
  protected void handleKeyEvent(KeyEvent event) {
    ...
    if (event.getID() == KeyEvent.KEY_PRESSED) {
      ...
      if (external && event.getModifiers() == MENU_SHORTCUT
          && event.getKeyCode() == 'W') {
        exit();
      }
    }
  ...
  /**
   * Overriding keyXxxxx(KeyEvent e) functions will cause the 'key', 'keyCode',
   * and 'keyEvent' variables to no longer work; key events will no longer be
   * queued until the end of draw(); and the keyPressed(), keyReleased() and
   * keyTyped() methods will no longer be called.
   */
   ...
   * stop the program for a half-second. Remember: the display window is updated
   * only at the end of draw(), so putting more than one delay() inside draw()
   * will simply add them together and the new frame will be drawn when the
   * total delay is over. <br/>
   * <br/>
   * I'm not sure if this is even helpful anymore, as the screen isn't updated
   * before or after the delay, meaning which means it just makes the app lock
   * up temporarily.
   */
   ...
   * frame so that the sketch synchronizes to a particular speed. Note that this
   * only sets the maximum frame rate, it cannot be used to make a slow sketch
   * go faster. Sketches have no default frame rate setting, and will attempt to
   * use maximum processor power to achieve maximum speed.
   */
      ...
      try {
        if (platform == WINDOWS) {
          ...
          try {
            ...
            Method openMethod = mrjFileUtils
   ...
   * make it easier to deal with spaces in the individual elements. (This avoids
   * the situation of trying to put single or double quotes around different
   * bits).
   */
  ...
  public void method(String name) {
    ...
    try {
      ...
      System.err.println("There is no " + name + "() method " + "in the class "
                         + getClass().getName());
    } catch (Exception e) {
      e.printStackTrace();
    }
  ...
  /*
   * public void thread(String name) { final Object o = this; final Class<?> c =
   * getClass(); try { final Method method = c.getMethod(name, new Class[] {});
   * Thread later = new Thread() { public void run() { try { method.invoke(o,
   * new Object[] { });
   * 
   * } catch (IllegalArgumentException e) { e.printStackTrace(); } catch
   * (IllegalAccessException e) { e.printStackTrace(); } catch
   * (InvocationTargetException e) { e.getTargetException().printStackTrace(); }
   * } }; later.start();
   * 
   * } catch (NoSuchMethodException nsme) { System.err.println("There is no " +
   * name + "() method " + "in the class " + getClass().getName());
   * 
  ...
  /**
   * Intercepts any relative paths to make them absolute (relative to the sketch
   * folder) before passing to save() in PImage. (Changed in 0100)
   */
  ...
  /**
   * Grab an image of what's currently in the drawing area and save it as a .tif
   * or .tga file.
   * <P>
   ...
   * <P>
   * The String passed in can contain a series of # signs that will be replaced
   * with the screengrab number.
   * 
   * <PRE>
  ...
  public void cursor(PImage image, int hotspotX, int hotspotY) {
    ...
    Image jimage = createImage(new MemoryImageSource(image.width, image.height,
  ...
  static public void println(Object what) {
    if (what == null) {
      ...
      if (name.charAt(0) == '[') {
         ...
         * < dd.length; i++) { System.out.println("[" + i + "] " + dd[i]); }
         * break;
         */
  ...
  /**
   * Normalize a value to exist between 0 and 1 (inclusive). Mathematically the
   ...
   * 'howbig'.
   * <P>
   * If howsmall is >= howbig, howsmall will be returned, meaning that random(5,
   * 5) will return 5 (useful) and random(7, 4) will return 7 (not useful..
   * better idea?)
   */
   ...
   * (including transparency), .tga, and .jpg images. In Java 1.3 or later, .png
   * images are <A
   * HREF="http://java.sun.com/j2se/1.3/docs/guide/2d/new_features.html"> also
   * supported</A>.
   * <P>
   * Generally, loadImage() should only be used during setup, because re-loading
   * images inside draw() is likely to cause a significant delay while memory is
   * allocated and the thread blocks while waiting for the image to load because
   * loading is not asynchronous.
   * <P>
   ...
   * <P>
   * As of 0096, returns null if no image of that name is found, rather than an
   * error.
   * <P>
   ...
   * use with Processing.
   * <P>
   * Also in release 0115, more image formats (BMP and others) can be read when
   * using Java 1.4 and later. Because many people still use Java 1.1 and 1.3,
   * these formats are not recommended for work that will be posted on the web.
   * To get a list of possible image formats for use with Java 1.4 and later,
   * use the following:
   * <TT>println(javax.imageio.ImageIO.getReaderFormatNames())</TT>
   * <P>
   * Images are loaded via a byte array that is passed to Toolkit.createImage().
   * Unfortunately, we cannot use Applet.getImage() because it takes a URL
   * argument, which would be a pain in the a-- to make work consistently for
   * online and local sketches. Sometimes this causes problems, resulting in
   * issues like <A
   * HREF="http://dev.processing.org/bugs/show_bug.cgi?id=279">Bug 279</A> and
   * <A HREF="http://dev.processing.org/bugs/show_bug.cgi?id=305">Bug 305</A>.
   * In release 0115, everything was instead run through javax.imageio, but that
   * turned out to be very slow, see <A
   * HREF="http://dev.processing.org/bugs/show_bug.cgi?id=392">Bug 392</A>. As a
   * result, starting with 0116, the following happens:
   * <UL>
   ...
   * to the PImage constructor that takes an AWT image.
   */
  ...
  /**
   * Identical to loadImage, but allows you to specify the type of image by its
   * extension. Especially useful when downloading from CGI scripts. <br/>
   * <br/>
   * Use 'unknown' as the extension to pass off to the default image loader that
   * handles gif, jpg, and png.
   */
  ...
  /**
   * Create a .vlw font on the fly from either a font name that's installed on
   * the system, or from a .ttf or .otf that's inside the data folder of this
   * sketch.
   * <P/>
   * Only works with Java 1.3 or later. Many .otf fonts don't seem to be
   * supported by Java, perhaps because they're CFF based?
   * <P/>
   * Font names are inconsistent across platforms and Java versions. On Mac OS
   * X, Java 1.3 uses the font menu name of the font, whereas Java 1.4 uses the
   * PostScript name of the font. Java 1.4 on OS X will also accept the font
   * menu name as well. On Windows, it appears that only the menu names are
   * used, no matter what Java version is in use. Naming system unknown/untested
   * for 1.5.
   * <P/>
   * Use 'null' for the charset if you want to use any of the 65,536 unicode
   * characters that exist in the font. Note that this can produce an enormous
   * file or may cause an OutOfMemoryError.
   */
  public PFont createFont(String name, float size, boolean smooth,
                          char charset[]) {
    ...
    try {
      SwingUtilities.invokeAndWait(new Runnable() {
        public void run() {
          ...
          selectedFile = (filename == null) ? null : new File(directory,
              filename);
        }
      });
      ...
      SwingUtilities.invokeAndWait(new Runnable() {
        public void run() {
          if (platform == MACOSX) {
            ...
            String filename = fileDialog.getFile();
            selectedFile = (filename == null) ? null : new File(fileDialog
  ...
  /**
   * I want to read lines from a file. I have RSI from typing these eight lines
   ...
   * OutputStream for a given filename or path. The file will be created in the
   * sketch folder, or in the same folder as an exported application.
   * <p/>
   * If the path does not exist, intermediate folders will be created. If an
   * exception occurs, it will be printed to the console, and null will be
   * returned.
   * <p/>
   * Future releases may also add support for handling HTTP POST via this method
   * (for better symmetry with createInput), however that's maybe a little too
   * clever (and then we'd have to add the same features to the other file
   * functions like createWriter). Who you callin' bloated?
   */
   ...
   * basically saveBytes(blah, loadBytes()), but done more efficiently (and with
   ...
   * "bytefun.txt" to a subfolder called 'poo' inside the sketch folder. If the
   * in-between subfolders don't exist, they'll be created.
   */
  ...
  /**
   * Calls System.arraycopy(), included here so that we can avoid people needing
   * to learn about the System object before they can just copy an array.
   */
  ...
  /**
   * Trim the whitespace from a String array. This returns a new array and does
   * not affect the passed-in array.
   */
   ...
   * whatever's passed in for the separator.
   * <P>
   * To use this on numbers, first pass the array to nf() or nfs() to get a list
   * of String objects, then use join on that.
   * 
   * <PRE>
   ...
   * confusing than necessary, and String.split() is always available for those
   * who want regexp.
   */
   ...
   * (Pattern.DOTALL) by default, so that ^ and $ match the beginning and end of
   * any lines found in the source, and the . operator will also pick up newline
   * characters.
   */
  ...
  /**
   * Identical to match(), except that it returns an array of all matches in the
   * specified String, rather than just the first.
   */
   ...
   * function just auto-size. If you want a specific number of digits (like all
   * 32) use binary(int what, int digits) to specify how many digits.
   */
  ...
  /**
   * Unpack a binary String into an int. i.e. unbinary("00001000") would return
   * 8.
   */
   ...
   */
  static public final byte[] ICON_IMAGE = { 71, 73, 70, 56, 57, 97, 16, 0, 16,
      0, -77, 0, 0, 0, 0, 0, -1, -1, -1, 12, 12, 13, -15, -15, -14, 45, 57, 74,
      54, 80, 111, 47, 71, 97, 62, 88, 117, 1, 14, 27, 7, 41, 73, 15, 52, 85,
      2, 31, 55, 4, 54, 94, 18, 69, 109, 37, 87, 126, -1, -1, -1, 33, -7, 4, 1,
      0, 0, 15, 0, 44, 0, 0, 0, 0, 16, 0, 16, 0, 0, 4, 122, -16, -107, 114,
      -86, -67, 83, 30, -42, 26, -17, -100, -45, 56, -57, -108, 48, 40, 122,
      -90, 104, 67, -91, -51, 32, -53, 77, -78, -100, 47, -86, 12, 76, -110,
      -20, -74, -101, 97, -93, 27, 40, 20, -65, 65, 48, -111, 99, -20, -112,
      -117, -123, -47, -105, 24, 114, -112, 74, 69, 84, 25, 93, 88, -75, 9, 46,
      2, 49, 88, -116, -67, 7, -19, -83, 60, 38, 3, -34, 2, 66, -95, 27, -98,
      13, 4, -17, 55, 33, 109, 11, 11, -2, -128, 121, 123, 62, 91, 120, -128,
      127, 122, 115, 102, 2, 119, 0, -116, -113, -119, 6, 102, 121, -108, -126,
      5, 18, 6, 4, -102, -101, -100, 114, 15, 17, 0, 59 };
   ...
   * </PRE>
   * 
   * This will properly launch your applet from a double-clickable .jar or from
   * the command line.
   * 
   * <PRE>
  ...
  static public void main(String args[]) {
    ...
    try {
       ...
       * Frame(displayDevice.getDefaultConfiguration()); } else { frame = new
       * Frame(); }
       */
      ...
      if (present) {
        ...
        if (exclusive) {
          ...
          fullScreenRect = new Rectangle(0, 0, mode.getWidth(), mode
              .getHeight());
      ...
      if (present) {
        ...
        int windowW = Math.max(applet.width, MIN_WINDOW_WIDTH) + insets.left
                      + insets.right;
        int windowH = Math.max(applet.height, MIN_WINDOW_HEIGHT) + insets.top
                      + insets.bottom;
  ...
  public void spotLight(float red, float green, float blue, float x, float y,
                        ...
                        float concentration) {
    if (recorder != null)
      recorder.spotLight(red, green, blue, x, y, z, nx, ny, nz, angle,
                         concentration);
  ...
  public void setCache(Object parent, Object storage) {
    if (recorder != null)
      recorder.setCache(parent, storage);
